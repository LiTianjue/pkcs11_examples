...
def password_callback(slot, password):
    if password:
        return password
    else:
...
def main():
	...
	parser.add_argument('-n', '--nickname', default=None)

	parser.add_argument('-w', '--db-passwd',
	...
	if options.use_connection_class:
		...
		conn = NSSConnection(url_components.netloc, 443, dbdir=options.db_name,nickname=options.nickname, password=options.db_passwd)
	    else:
